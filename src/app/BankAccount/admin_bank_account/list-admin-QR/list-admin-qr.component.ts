...
export class ListAdminQRComponent implements OnInit {
  ...
  Add_Admin_QR_Popup(): void {
    ...
    this.bankaccountservice.OpenAddAdminQRPopup(false, payload, this.selectedSiteName);
    this.bankaccountservice.bsmodalRef?.onHidden?.subscribe(() => this.list_admin_QRs());
  }

  Make_Admin_QR_Default(obj: any): void {
    this.makedefaultservice.OpenMakeDefaultPopup('adminqr', 'QR', obj);
  }

  Delete_Admin_QR(obj: any): void {
  ...
  list_admin_QRs(): void {
    ...
    this.bankaccountservice.list_admin_QR_accounts(this.selectedSiteId).subscribe({
      next: (response) => {
