...
@Component({
  selector: 'app-withdraw-from-site-history',
  templateUrl: './withdraw-from-site-history.component.html',
  styleUrls: ['./withdraw-from-site-history.component.css', '../coins-listing.shared.css']
})
export class WithdrawFromSiteHistoryComponent implements OnInit {
  ...
  loadHistory(): void {
    ...
    this.loading = true;
    this.coinsservice.withdraw_from_site_history(0, this._sessionUser).subscribe({
      next: (resp: any) => {
        this.loading = false;
        this.requestList = resp?.returnList ?? [];
        if (resp?.returnStatus != null && Number(resp.returnStatus) !== 1 && this.requestList.length === 0) {
          this.toasterService.warning(
            resp.returnMessage || 'Withdraw history unavailable. Deploy USP_ListCoinsToAccountRequestHistory and restart the API.'
          );
        }
      },
      error: (err) => {
        ...
        const msg =
          ...
          err?.message ||
          'Unable to load withdraw history. Confirm the API is running and USP_ListCoinsToAccountRequestHistory is deployed.';
